...
def squash(vectors, axis=-1):
    ...
    """
    #sigmoid-squash
    x= tf.sigmoid(vectors)
    s_squared_norm = tf.reduce_sum(tf.square(vectors), axis, keepdims=True)
    return (x*vectors)/tf.sqrt(s_squared_norm+K.epsilon())
...
class CapsuleLayer(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        inputs_hat = tf.matmul(self.W, inputs_tiled)
        #inputs_hat = tf.stop_gradient(inputs_hat)
        ...
        b = tf.random.uniform(shape=[tf.shape(inputs_hat)[0], self.num_capsule, 1, self.input_num_capsule], minval=0, maxval=1)
        #b = tf.zeros(shape=[tf.shape(inputs_hat)[0], self.num_capsule, 1, self.input_num_capsule])
        #b=tf.concat(inputs_hat,b)
        #assert self.routings > 0, 'The routings should be > 0.'
        outputs=inputs_hat
        for i in range(self.routings):
            # c.shape=[batch_size, num_capsule, 1, input_num_capsule]
            c=tf.exp(b)/(tf.reduce_mean(tf.exp(b),axis=0, keepdims=False))
